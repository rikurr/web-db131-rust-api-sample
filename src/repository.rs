...
impl Repository {
    ...
    pub async fn delete_post(&self, id: i32) -> Result<(), ApiError> {
        let mut conn = self.pool.get()?;
        web::block(move || diesel::delete(posts::table.find(id)).execute(&mut conn)).await??;
        Ok(())
    }

    pub async fn update_post(&self, id: i32, changeset: PostChangeset) -> Result<Post, ApiError> {
        let mut conn = self.pool.get()?;
    ...
    pub async fn published_post(&self, id: i32, published: bool) -> Result<(), ApiError> {
        let mut conn = self.pool.get()?;
        web::block(move || {
            diesel::update(posts::table.find(id))
